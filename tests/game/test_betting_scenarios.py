...
def test_betting_round_raise_and_call(mock_logger, mock_game, setup_players):
    ...
    randy_decision = ActionDecision(
        ...
        reasoning="Decent hand"
    )

    # Configure decisions and executions
    players[0].decide_action = MagicMock(return_value=alice_decision)
    players[0].execute = MagicMock(side_effect=lambda d, g: players[0].place_bet(d.raise_amount, g))
    
    players[1].decide_action = MagicMock(return_value=bob_decision)
    players[1].execute = MagicMock(side_effect=lambda d, g: setattr(players[1], 'folded', True))
    
    players[2].decide_action = MagicMock(return_value=charlie_decision)
    players[2].execute = MagicMock(side_effect=lambda d, g: setattr(players[2], 'folded', True))
    
    players[3].decide_action = MagicMock(return_value=randy_decision)
    players[3].execute = MagicMock(side_effect=lambda d, g: players[3].place_bet(300, g))
    ...
    assert len(player_queue.needs_to_act) == 0, "needs_to_act should be empty after round completion"
    ...
    assert players[0] in player_queue.acted_since_last_raise, "Alice should be in acted_since_last_raise"
    assert players[1] in player_queue.acted_since_last_raise, "Bob should be in acted_since_last_raise"
    assert players[2] in player_queue.acted_since_last_raise, "Charlie should be in acted_since_last_raise"
    assert players[3] in player_queue.acted_since_last_raise, "Randy should be in acted_since_last_raise"
...
def test_betting_round_raise_reraise_call(mock_logger, mock_game, setup_players):
    ...
    def alice_actions(game):
        ...
        return ActionDecision(
            action_type=ActionType.CALL,
            reasoning="Still strong but not raising"
        )
    ...
    players[1].decide_action = MagicMock(return_value=ActionDecision(
        action_type=ActionType.RAISE,
        raise_amount=600,
        reasoning="Stronger hand"
    ))
    players[1].execute = MagicMock(side_effect=lambda d, g: players[1].place_bet(d.raise_amount, g))
    
    players[2].decide_action = MagicMock(return_value=ActionDecision(
        action_type=ActionType.FOLD,
        reasoning="Weak hand"
    ))
    players[2].execute = MagicMock(side_effect=lambda d, g: setattr(players[2], 'folded', True))
    
    players[3].decide_action = MagicMock(return_value=ActionDecision(
        action_type=ActionType.FOLD,
        reasoning="Weak hand"
    ))
    players[3].execute = MagicMock(side_effect=lambda d, g: setattr(players[3], 'folded', True))
